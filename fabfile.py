...
from brawler.settings.local import DATABASES as LOCAL_DB
import os
...
def run_test():
    local("coverage run ./manage.py test --settings='brawler.settings.testing'")
    local("coverage html")
...
def clonedigger_run(filename='clonedigger_output.html'):
    conf = importlib.import_module('brawler.settings.local')
    ignore_dirs = ''
...
class ServerUtil(object):
    ...
    def __init__(self, settings):
        ...
        self.conf = importlib.import_module('brawler.settings.{}'.format(settings))
